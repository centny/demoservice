module github.com/centny/demoservice

go 1.17

require (
	github.com/Centny/rediscache v0.0.0-20220105111036-c599d3d485bd
	github.com/codingeasygo/util v0.0.0-20220224082857-541d616b7ffd
	github.com/codingeasygo/web v0.0.0-20211223064502-c2f8e0e9af2f
	github.com/gomodule/redigo v1.8.8
	github.com/jackc/pgconn v1.11.0
	github.com/jackc/pgx/v4 v4.15.0
	go.uber.org/zap v1.21.0
	golang.org/x/net v0.0.0-20220225172249-27dd8689420f
	sxbastudio.com/base/go v0.0.0-20211110083116-9b05554995f7
)

require (
	github.com/axw/gocov v1.0.0 // indirect
	github.com/jackc/chunkreader/v2 v2.0.1 // indirect
	github.com/jackc/pgio v1.0.0 // indirect
	github.com/jackc/pgpassfile v1.0.0 // indirect
	github.com/jackc/pgproto3/v2 v2.2.0 // indirect
	github.com/jackc/pgservicefile v0.0.0-20200714003250-2b9c44734f2b // indirect
	github.com/jackc/pgtype v1.10.0 // indirect
	github.com/jackc/puddle v1.2.1 // indirect
	github.com/matm/gocov-html v0.0.0-20200509184451-71874e2e203b // indirect
	go.uber.org/atomic v1.7.0 // indirect
	go.uber.org/multierr v1.6.0 // indirect
	golang.org/x/crypto v0.0.0-20210711020723-a769d52b0f97 // indirect
	golang.org/x/text v0.3.7 // indirect
	golang.org/x/tools v0.1.5 // indirect
)
